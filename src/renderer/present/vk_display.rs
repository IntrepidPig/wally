...
impl SurfaceCreator for DisplaySurfaceCreator {
	type CreateArgs = (); // TODO modeset selection closure
	...
	unsafe fn create_surface(&mut self, physical_device: vk::PhysicalDevice, device: &Device) -> Result<Self::SurfaceOwner, ()> {
		let displays_properties = self.display_loader.get_physical_device_display_properties(physical_device)
		...
		let display_planes_properties = self.display_loader.get_physical_device_display_plane_properties(physical_device).unwrap();
		let (display_plane_index, display_plane_properties) = display_planes_properties
			.iter()
			.enumerate()
			.find(|(index, properties)| {
				let supported_displays = self.display_loader.get_display_plane_supported_displays(physical_device, *index as u32).unwrap();
				supported_displays.iter().any(|display| *display == display_properties.display)
			})
			.map(|(index, properties)| (index as u32, properties))
			.unwrap();
		let display_modes_properties = self.display_loader.get_display_mode_properties(physical_device, display_properties.display).unwrap();
		let display_mode_properties = display_modes_properties[0];
		let display_plane_capabilities = self.display_loader.get_display_plane_capabilities(physical_device, display_mode_properties.display_mode, display_plane_index).unwrap();
		...
		let surface = self.display_loader.create_display_plane_surface(&surface_create_info, None).unwrap();
