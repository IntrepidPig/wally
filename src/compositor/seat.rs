...
impl<I: InputBackend, G: GraphicsBackend> CompositorState<I, G> {
	...
	pub fn handle_pointer_release(&mut self, this: Resource<WlPointer, ()>) {
		this.client().get().unwrap().state::<RefCell<ClientState<G>>>().borrow_mut().pointers.retain(|pointer| !pointer.is(&this));
		this.destroy();
	}
	...
	pub fn handle_keyboard_release(&mut self, this: Resource<WlKeyboard, ()>) {
		this.client().get().unwrap().state::<RefCell<ClientState<G>>>().borrow_mut().keyboards.retain(|pointer| !pointer.is(&this));
		this.destroy();
	}
